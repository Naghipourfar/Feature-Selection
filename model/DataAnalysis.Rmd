...
runs_data <- data.frame(MSE=c(0.0), SD=c(0.0)) # initialize variables
```
...
distplot <- function(k) {
  ...
  data <- read_csv(file_name, col_names = c("MSE"))
...
qplot(x = c(0, 2, 4, 8, 16, 32, 64, 128, 256, 512), y = runs_data$MSE) + geom_errorbar(aes(ymin=y-sd, ymax=y+sd), width=0.5)
