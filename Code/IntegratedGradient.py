...
class integrated_gradients:
    # Code: Keras Code that you wish to explain.
    # outchannels: In case the Code are multi tasking, you can specify which output you want explain .
    def __init__(self, model, outchannels=[], verbose=1):

        # get backend info (either tensorflow or theano)
        self.backend = K.backend()

        # load Code supports keras.Model and keras.Sequential
        ...
        for c in self.outchannels:
            ...
            # This takes a lot of time for a big Code with many tasks.
